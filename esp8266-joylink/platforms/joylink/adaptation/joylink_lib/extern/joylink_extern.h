...
typedef int (*E_JL_DEV_ATTR_GET_CB)(char *buf, unsigned int buf_sz);

typedef int (*E_JL_DEV_ATTR_SET_CB)(const char *json_in);

/**
 * check if connect to ap, (but don't make sure if connect to internet)
 */
E_JLRetCode_t
joylink_dev_is_net_ok();

/**
 *set the server connect status
 */
E_JLRetCode_t
joylink_dev_set_connect_st(int st);

/**
 * save Joylink config info
 */
E_JLRetCode_t
joylink_dev_set_attr_jlp(JLPInfo_t *jlp);

/**
 * get joylink config info
 */
E_JLRetCode_t
