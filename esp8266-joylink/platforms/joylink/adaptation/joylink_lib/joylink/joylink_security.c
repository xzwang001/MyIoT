        ...
        int length)
{
	...
	if(NULL == (peerPubKey = (uint8_t *)zalloc(uECC_BYTES * 2))){
		return retlen;
	}

	if(NULL == (secret = (uint8_t *)zalloc(uECC_BYTES))){
		return retlen;
	}
	...
	case ET_ECDH:
		...
		pOut += (uECC_BYTES + 1);
		uECC_decompress_joylink(key, peerPubKey);

		uECC_shared_secret_joylink(
                peerPubKey, 
                __g_ekey.priKey, 
                secret);
        ...
        int payloadLen)
{
	...
	case PT_AUTH:
		...
		pOut += 4;
		log_debug("PT_AUTH:%s, payloadlen %d,unm %d, time %d\n",\
            payload, payloadLen, ap->random_unm, ap->timestamp);
        ...
        uint8_t* pOut, int maxlen)
{
	...
	case PT_AUTH:
		...
		log_debug("AUTH:accesskey to work:%s\n",_g_pdev->jlp.accesskey);
...
joylink_dencypt_lan_req(JLPacketParam_t *pParam, const uint8_t *pIn, int length, uint8_t* pOut, int maxlen)
{
	...
	if (0x123455bb == pCommon->magic){
		...
		case ET_ACCESSKEYAES:
            ...
            break;
		case ET_ECDH:
            uECC_decompress_joylink(pIn + sizeof(JLPacketHead_t), devPubKey);
            uECC_shared_secret_joylink(devPubKey, __g_ekey.priKey, secret);
