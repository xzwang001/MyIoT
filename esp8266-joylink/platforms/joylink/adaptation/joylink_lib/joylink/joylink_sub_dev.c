...
//    char data[JL_MAX_PACKET_LEN];
    ...
    ret =  joylink_dencrypt_sub_dev_data(src + offset, src_len - offset, 
            ...
            (uint8_t *)data, JL_MAX_PACKET_LEN);
    ...
    switch(ctr->biz_code){
        ...
        case JL_BZCODE_CTRL:
            ...
            if(ret > 0){
                ...
                ctr->cmd[ret] = 0;
                log_info("feedid:%s:cmd:%s:ret:%d", ctr->feedid, ctr->cmd, ret);
            }else{
                log_error("cmd dencrypt error:%s:%s", ctr->feedid, recPainText + 44);
            }
...
joylink_packet_subdev_script_ctrl_rsp(char *data, int max, JLSubContrl_t *ctrl)
{
    ...
    if(ctrl->biz_code == JL_BZCODE_GET_SNAPSHOT || ctrl->biz_code == JL_BZCODE_CTRL){
        ...
        log_debug("data of length:%d", ret);
