...
int 
joylink_packet_script_ctrl_rsp(char *data, int max, JLContrl_t *ctrl, uint8_t to_server)
{
    ...
    if(ctrl->biz_code == JL_BZCODE_GET_SNAPSHOT){
        ...
        ctrl->biz_code = 102;
    }else if(ctrl->biz_code == JL_BZCODE_CLOUD_MENU){
        ctrl->biz_code = 105;
    }else{
        log_error("else biz code %d", ctrl->biz_code);
    }
    ...
    ret = joylink_dev_get_snap_shot(data + 12, JL_MAX_PACKET_LEN - 12, to_server);
