...
uint8 joylink_softap_getCrc(uint8 *ptr, uint8 len)
{
	unsigned char crc;
	unsigned char i;
	crc = 0;
...
uint16 joylink_softap_auth_crc16(const uint8 *buffer, uint32 size)
{
...
void joylink_softap_dump8(uint8* p, int split, int len)
{
	int i;
	char buf[512];
	int index = 0;
	...
	log_debug("Addr=%d,Len=%d:%s", p, len, buf);
}

int joylink_softap_util_byte2hexstr(const uint8 *pbytes, int blen, uint8 *o_phex, int hlen)
{
...
int joylink_softap_util_make_guid(uint8 *guid_str, uint8 max_len)
{
	guid_t guid;	
	int	guid_len = -1;
	...
	if(guid_str && max_len){
		...
		guid_len = joylink_softap_util_byte2hexstr(guid_hex, 16, guid_str, max_len);
	}
	
	return guid_len;
}

int joylink_softap_AesCBC256Decrypt(uint8 *pEncIn, uint32 encLength,uint8 *key_32bytes, uint8 *pPlainOut, uint32 maxPlainLength)
{
	uint8 key[16] = { 0 };
	uint8 iv[16] = { 0 };

	log_debug("aes encrypt_enclen = 0x%x:",encLength);
	print_buf(pEncIn,encLength);
    log_debug("key_32bytes:");
    print_buf(key_32bytes, 32);
    log_debug("maxPlainLength:0x%x", maxPlainLength);
	...
	{
		log_debug("decrypt data max than expect:%d,%d",ret,maxPlainLength);
		return 0;
	}

	log_debug("decrypt data len :%x",ret);
	memcpy(pPlainOut, Out, ret);
	return ret;
	
}

int joylink_softap_AesCBC256Encrypt(uint8 *pEncIn, int32 encLength,uint8 *key_32bytes, uint8 *pPlainOut, uint32 maxPlainLength)
{
	uint8 key[16] = { 0 };
	uint8 iv[16] = { 0 };
	memcpy(iv,key_32bytes,16);
...
int joylink_softap_uECC_generate_rng(uint8 *buf,unsigned size)
{
	unsigned i;
